...
function Registration({ setIsRegistrired }: { setIsRegistrired: React.Dispatch<React.SetStateAction<boolean>> }) {
    interface FormData {
        email: string;
        password: string;
    ...
    return (
        <div ><img src={logo} alt="" className="m-auto block mt-[30px]" />
            <div className="max-w-[327px] sm:max-w-[340px] md:max-w-[400px] w-[90%] bg-loginBg px-[20px] py-[30px] m-auto rounded-[8px] mt-[50px]">
                    ...
                    <div className="mt-[25px] mb-[40px] flex flex-col gap-[20px]">
                        ...
                        <div>
                            <div className="relative">
                                ...
                                })} onChange={(e) => setPassword(e.target.value)} value={password} className="pl-[15px] pb-[10px] mb-[20px] w-[100%] border-b border-solid border-greilishBlue block bg-[initial]  text-primaryText outline-none" placeholder="Password" type="password" name="password" />
                                {watchedPassword && errors.password?.message && <span className="text-buttonBg absolute top-0 left-[60%] md:left-[68%]">Can't be empty</span>}
                                ...
                                })} className="pl-[15px] pb-[10px] w-[100%] border-b border-solid border-greilishBlue block bg-[initial]  text-primaryText outline-none" placeholder="Repeat password" type="password" name="repeatPassword" />
                                {watchedPassword !== watchedRepeatPas && <span className="text-buttonBg absolute text-[12px] top-[54px] left-[55%] md:left-[64%]">Passwords do not match</span>}
                            </div>
                ...
                <h4 className="text-primaryText mt-[20px] text-center">Don’t have an account? <span onClick={() => setIsRegistrired(true)} className="text-buttonBg text-[15px] cursor-pointer">Sign In</span></h4>
            </div></div>
    )
}
